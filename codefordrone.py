...
## !!!!! TRIED TO CHANGE THE WHOLE BLURRED GREEN AND BLURRED BROWN, BUT COULDN'T CHANGE TO SOLID COLOR !!!!!!!




#Create masks for burnt (brown) and unburnt (green) areas
#burnt_mask = cv2.inRange(image, brown_lower, brown_upper)
#unburnt_mask = cv2.inRange(image, green_lower, green_upper)

# Count houses in the burnt and unburnt areas
#houses_in_burnt = count_houses(burnt_mask)
#houses_in_unburnt = count_houses(unburnt_mask)

# Create solid color images for burnt and unburnt areas
#burnt_color = np.zeros_like(image)
#burnt_color[burnt_mask != 0] = [0, 255, 255]  # yellow color for burnt areas

#unburnt_color = np.zeros_like(image)
#unburnt_color[unburnt_mask != 0] = [128,0, 128]   # Purple color for unburnt areas

# Merge the solid color images with the original image
#result_image = cv2.add(image, burnt_color)
#result_image = cv2.add(result_image, unburnt_color)
